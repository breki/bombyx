...
were. The VM host now holds no project code, which is half of
the first statement. The workstation holds the other half: you
run bombyx from inside a checkout, because `bombyx.toml` is read
from the working directory, and that working directory is a
clone of the project. One file blocks both statements, and
`project-config-off-repo` is the work that moves it.

## Where project code lives today

The VM host holds no project code. The workstation still holds a
checkout, and the guest clones its own copy.
...
Three machines held project files and now one does. Vagrant
...
project. That is the machine `README.md` opens by saying you
should keep project code off, and it is what
`project-config-off-repo` addresses.
