...
> is gone, so no machine outside the guest holds the project's
> source. What has not: the workstation still reads
> `bombyx.toml` out of the project's working directory.
>
...
is read from the working directory, so the workstation still
depends on a file the repository ships.

## Where project code lives today

The guest holds the project's source and no other machine does.
...
There were three machines holding project files, and each was
removed for its own reason. Vagrant mounts the Vagrantfile's
directory at `/vagrant` unless told otherwise, which put the VM
host's copy inside the guest; the generated Vagrantfile disables
that share. The VM host's own copy arrived in a `tar` archive
that no program there read once the share was disabled, and that
push is now gone. The workstation no longer builds an archive,
so it needs no checkout for that.

What the workstation still reads from the project's directory is
`bombyx.toml`. That is the remaining gap, and it is what the
second statement above is about.
...
- `remote-clone-project-source` -- the bootstrap clones
  ...
  commands and the first statement above is reached.
