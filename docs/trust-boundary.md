...
A configured `deploy_key` adds a fifth, ahead of all of these:
an `ssh` that tests the key file is on the VM host and exits
non-zero when it is not. It reads one file's presence and never
its contents, so it moves no project code anywhere.
...
What changed is that bombyx now has a mechanism for it, so the
exposure is a described one rather than a gap. `deploy_key` in
`[source]` names a private key file **on the VM host**. Before
...
Three properties are worth stating, because each is a
...
SSH user, which is `vagrant` -- the user the agent works as --
so a key left where it lands is one the agent's own code reads
with no effort. Root-owned and `0600` means reading it takes
root. On most Vagrant boxes the `vagrant` user has passwordless
`sudo`, so code running in the VM can still get there. What it
buys is that the theft is an escalation rather than a `cat`.
...
a change afterwards. An attacker positioned between the guest
and the git host on that first connection can present their
own key and receive the deploy key's authentication attempt.
The egress rules under `host-network-isolation` are what would
narrow that, and they are not loaded.

Removing `deploy_key` from the config removes the key from the
guest: `bootstrap.sh` deletes `/root/.ssh/bombyx-deploy-key`
whenever no upload replaced it, so a credential nobody
remembers granting does not survive in a VM that no longer
needs one.
...
- `remote-clone-project-source` -- the bootstrap clones
  ...
  commands -- five with a `deploy_key` configured -- and the VM
...
`guest-credential-for-private-repo` is the work that added it.
