...
- `docs/architecture.md`'s domain-entity class diagram, plus the
  newtype constructors under `config/`, which together are the
  inventory a reader consults to answer which fields reach `ssh`
  unchecked
That last one is the one nobody opens. A `/review` on #25
found `docs/architecture.md` claiming `Project` had no `host`
field on the branch that added one. It was not in any snapshot,
because it had not been touched -- while every earlier step of
that same series had updated it alongside the code.
