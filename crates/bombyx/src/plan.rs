...
mod tests {
    ...
    /// credential as well.
    fn staged(with_token: bool) -> (Config, Staged) {
    ...
    fn a_configured_env_file_is_written_and_then_removed() {
        ...
        for action in [
            ...
            let cmds = {
                let (cfg, staged) = staged(true);
                plan(&action, &cfg, Tty::NoPty, &staged)
            };
            let write = cmds
                .iter()
    ...
    fn the_credential_write_does_not_print_its_size() {
        ...
        let cmds = {
            let (cfg, staged) = staged(true);
            plan(&Action::Up, &cfg, Tty::NoPty, &staged)
        };
        let cred = cmds
            .iter()
